"""Continue the selected CNN Task3-P family through Task3-C, then stop."""
...
DEFAULT_PROTOCOL = ROOT / "experiments/configs/model-a-cnn-n8-task3-coin-s150000.json"
KIND_TRAIN = "model-a-cnn-n8-task3-coin-training"
KIND_EVAL = "model-a-cnn-n8-task3-coin-evaluation"
KIND_REPORT = "model-a-cnn-n8-task3-coin-report"
...
def load_protocol(path: Path) -> tuple[dict, str]:
    ...
    if (protocol.get("kind") != "model-a-cnn-n8-clean-curriculum"
            or protocol.get("authorized_course") != "task3_coin"
            or protocol.get("next_course_started")):
        raise RuntimeError("wrong Task3-C protocol")
    return protocol, hashlib.sha256(raw).hexdigest()


def validate_task3p_lineage(protocol: dict) -> None:
    report_path = ROOT / protocol["task3p_parent_report"]["path"]
    if sha256_file(report_path) != protocol["task3p_parent_report"]["sha256"]:
        raise RuntimeError("Task3-P parent report drift")
    report = json.loads(report_path.read_text(encoding="utf-8"))
    if report.get("status") != "completed" or report["selection"]["selected_common_milestone"] != 600:
        raise RuntimeError("Task3-P report does not authorize these parents")
    for replica, spec in protocol["parents"].items():
        path = ROOT / spec["path"]
        if sha256_file(path) != spec["sha256"] or report["selected_checkpoints"][replica] != spec:
            raise RuntimeError(f"Task3-P parent drift for {replica}")
        payload = _torch_load(path)
        if (payload.get("architecture") != ARCHITECTURE or payload.get("stage") != "task3_peaceful"
                or payload.get("replica") != replica):
            raise RuntimeError(f"Task3-P parent identity mismatch for {replica}")
...
def validate_checkpoint(protocol: dict, protocol_hash: str, replica: str, milestone: int) -> dict:
    ...
    expected = {
        "architecture": ARCHITECTURE, "protocol_sha256": protocol_hash, "stage": "task3_coin",
        "replica": replica, "stage_rounds": milestone, "completed_total_rounds": 1800 + milestone,
        "n_step": 8, "parent_sha256": protocol["parents"][replica]["sha256"],
    }
    for key, value in expected.items():
        if payload.get(key) != value:
            raise RuntimeError(f"Task3-C checkpoint {key} mismatch for {replica}/round-{milestone}")
    ...
    if (diagnostics["raw_transitions"] != diagnostics["matured_targets"]
        ...
        raise RuntimeError("Task3-C transition accounting mismatch")
    return payload
...
def train_one(protocol_path: Path, protocol: dict, protocol_hash: str, replica: str) -> dict:
    ...
    if existing is not None:
        for milestone in protocol["task3_coin"]["milestones"]:
            ...
            if existing["checkpoints"][str(milestone)]["sha256"] != sha256_file(path):
                raise RuntimeError("completed Task3-C checkpoint drift")
        return existing
    ...
    if stats.exists() or any(checkpoint_dir.glob("round-*.pt")):
        raise RuntimeError(f"orphaned Task3-C artifact for {replica}")
    seed = protocol["task3_coin"]["replicas"][replica]; parent = protocol["parents"][replica]
    command = [sys.executable, "main.py", "play", "--agents", "model_a_cnn_n8", "seeded_coin_collector_agent",
               "--train", "1", "--continue-without-training", "--no-gui", "--scenario", "classic",
               "--n-rounds", str(protocol["task3_coin"]["rounds_per_replica"]),
               "--seed", str(seed["world_seed"]), "--save-stats", str(stats)]
    overrides = {
        "MODEL_A_CNN_PROTOCOL_PATH": str(protocol_path), "MODEL_A_CNN_MODE": "train",
        "MODEL_A_CNN_STAGE": "task3_coin", "MODEL_A_CNN_REPLICA": replica,
    ...
    try:
        if completed.returncode or not stats.is_file():
            raise RuntimeError("Task3-C training process failed")
        checkpoints = {}
        for milestone in protocol["task3_coin"]["milestones"]:
            path = checkpoint_path(protocol, replica, int(milestone))
            validate_checkpoint(protocol, protocol_hash, replica, int(milestone))
    ...
    if record["status"] != "completed":
        raise RuntimeError(f"Task3-C training failed for {replica}: {record.get('error')}")
    return record
...
def primary_labels(protocol: dict) -> dict[str, tuple[str, Path, str, str]]:
    result = baseline_labels(protocol, "task3_coin")
    for replica in protocol["task3_coin"]["replicas"]:
        for milestone in protocol["task3_coin"]["milestones"]:
            result[f"cnn-{replica}-round{int(milestone):04d}"] = (
                "model_a_cnn_n8", checkpoint_path(protocol, replica, int(milestone)), replica, "task3_coin")
    return result
...
def retention_labels(protocol: dict, selected: int, stage: str) -> dict[str, tuple[str, Path, str, str]]:
    ...
    for replica, parent in protocol["parents"].items():
        result[f"task3p-parent-{replica}"] = ("model_a_cnn_n8", ROOT / parent["path"], replica, "task3_peaceful")
        result[f"task3c-selected-{replica}"] = (
            "model_a_cnn_n8", checkpoint_path(protocol, replica, selected), replica, "task3_coin")
    return result
...
def evaluate_one(protocol_path: Path, protocol: dict, protocol_hash: str, suite: str, suite_spec: dict,
    ...
    if existing is not None:
        if existing["checkpoint"]["sha256"] != checkpoint_hash or existing["case"] != case:
            raise RuntimeError(f"Task3-C evaluation drift: {relative(manifest)}")
        return existing
    stats = manifest.with_suffix(".stats.json")
    if stats.exists():
        raise RuntimeError(f"orphaned Task3-C evaluation stats: {relative(stats)}")
    ...
    try:
        if completed.returncode or not stats.is_file() or sha256_file(checkpoint) != checkpoint_hash:
            raise RuntimeError("Task3-C evaluation process failed or mutated checkpoint")
    ...
    atomic_json(manifest, record)
    if record["status"] != "completed":
        raise RuntimeError(f"Task3-C evaluation failed: {suite}/{label}/{case['world_seed']}")
    return record
...
def execute(protocol_path: Path, protocol: dict, protocol_hash: str, hardware: dict) -> dict:
    validate_task3p_lineage(protocol)
    for replica in protocol["task3_coin"]["replicas"]:
        train_one(protocol_path, protocol, protocol_hash, replica)
    primary = evaluate_suite(protocol_path, protocol, protocol_hash, "task3c_selection", primary_labels(protocol))
    selection = select_common_milestone(primary, [int(x) for x in protocol["task3_coin"]["milestones"]],
                                        float(protocol["selection"]["near_best_tolerance_per_round"]))
    selected = int(selection["selected_common_milestone"])
    retention = {
        "task3_peaceful": evaluate_suite(protocol_path, protocol, protocol_hash, "task3p_retention",
                                          retention_labels(protocol, selected, "task3_peaceful")),
    ...
    report = {"schema_version": 1, "kind": KIND_REPORT, "protocol_id": protocol["protocol_id"],
              ...
              "course": "task3_coin", "status": "completed", "completed_at_utc": utc_now(),
              "hardware_preflight": hardware, "task3c_validation_by_label": primary, "selection": selection,
              "selected_checkpoints": {replica: {"path": relative(checkpoint_path(protocol, replica, selected)),
                                       ...
                                       for replica in protocol["task3_coin"]["replicas"]},
              "retention_by_task": retention, "next_course": "task4", "awaiting_user_instruction": True,
              "next_course_started": False, "default_model_replaced": False, "automatic_followup_started": False}
    output = ROOT / protocol["report_path"]
    if output.exists():
        raise RuntimeError(f"refusing to overwrite Task3-C report: {relative(output)}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    protocol, protocol_hash = load_protocol(protocol_path); validate_task3p_lineage(protocol)
    ...
    if args.execute:
        ...
        result = {"mode": "dry-run", "protocol_id": protocol["protocol_id"], "protocol_sha256": protocol_hash,
                  ...
                  "training_rounds": protocol["task3_coin"]["total_training_rounds"],
                  "evaluation_rounds": protocol["validation"]["total_rounds"],
                  "will_stop_after": "Task3-C selection, Task1/Task2/Task3-P retention, and Task3-C report",
                  "will_not_do": ["Task4", "checkpoint deployment", "final confirmation"]}
