"""Run the corrected terminal Task4C matched A/B for six-step BOMB credit."""
...
from agent_code.model_a_v4_bomb_credit6.config import (  # noqa: E402
...
DEFAULT_PROTOCOL = ROOT / "experiments/configs/model-a-v4-task4-bomb-credit6-s131000.json"
TRAINING_KIND = "model-a-v4-task4-bomb-credit6-training"
EVALUATION_KIND = "model-a-v4-task4-bomb-credit6-evaluation"
REPORT_KIND = "model-a-v4-task4-bomb-credit6-report"
...
def assert_registered_seeds_untouched(protocol_path: Path, protocol: dict) -> None:
    ...
    if conflicts:
        raise ValueError(f"registered s131000 seeds were already used: {conflicts}")


def _torch_load(path: Path) -> dict:
    from agent_code.model_a_v4_bomb_credit6.callbacks import _torch_load as load
    return load(path)


def validate_checkpoint(protocol: dict, protocol_hash: str, arm: str, replica: str, path: Path, stage_round: int) -> dict:
    if not path.is_file():
        raise RuntimeError(f"BOMB-credit6 checkpoint missing: {path}")
    ...
    expected = {
        ...
        "bomb_return_steps": 1 if arm == "control" else 6,
    ...
    if abs(float(payload.get("epsilon", -1.0)) - 0.05) > 1e-12:
        raise RuntimeError(f"BOMB-credit6 epsilon changed: {path}")
    diagnostics = payload.get("target_diagnostics", {})
    required = {
        "raw_transitions", "non_bomb_one_step_targets", "bomb_targets_matured",
        "bomb_targets_full_six_step_observed", "bomb_targets_terminal_truncated",
        ...
        "uniquely_attributed_kill_events", "uniquely_attributed_self_events",
        "outcome_lag_histogram", "terminal_deferred_kill_events", "attribution_errors",
    }
    if set(diagnostics) != required:
        raise RuntimeError(f"BOMB-credit6 target diagnostics changed: {path}")
    if diagnostics["raw_transitions"] != diagnostics["non_bomb_one_step_targets"] + diagnostics["bomb_targets_matured"]:
        raise RuntimeError(f"BOMB-credit6 transition accounting mismatch: {path}")
    if diagnostics["attribution_errors"] != 0:
        raise RuntimeError(f"BOMB-credit6 outcome attribution failed: {path}")
    for observed, attributed in (
        ...
        if diagnostics[observed] != diagnostics[attributed]:
            raise RuntimeError(f"BOMB-credit6 outcome coverage mismatch: {path}")
    lag_histogram = diagnostics["outcome_lag_histogram"]
    if set(lag_histogram) != {"4", "5", "terminal_deferred"} or sum(lag_histogram.values()) != diagnostics["observed_kill_events"] + diagnostics["observed_self_events"]:
        raise RuntimeError(f"BOMB-credit6 outcome lag accounting mismatch: {path}")
    if lag_histogram["terminal_deferred"] != diagnostics["terminal_deferred_kill_events"]:
        raise RuntimeError(f"BOMB-credit6 deferred terminal outcome mismatch: {path}")
    ...
    if set(histogram) != {str(i) for i in range(1, 7)} or sum(histogram.values()) != diagnostics["bomb_targets_matured"]:
        raise RuntimeError(f"BOMB-credit6 return histogram mismatch: {path}")
    if arm == "control":
        if histogram["1"] != diagnostics["bomb_targets_matured"] or any(histogram[str(i)] for i in range(2, 7)):
        ...
        if histogram["6"] != diagnostics["bomb_targets_full_six_step_observed"]:
...
def validate_training_artifacts(protocol: dict, protocol_hash: str, arm: str, replica: str) -> list[dict]:
    ...
    if sha256_file(final) != result[-1]["sha256"]:
        raise RuntimeError(f"BOMB-credit6 final is not immutable round-100: {arm}/{replica}")
    return result
...
def train_one(protocol_path: Path, protocol: dict, protocol_hash: str, arm: str, replica: str) -> dict:
    ...
    if existing is not None:
        if existing.get("protocol_sha256") != protocol_hash:
            raise RuntimeError(f"completed BOMB-credit6 training protocol drift: {arm}/{replica}")
    ...
    if output.parent.exists() or stats_path.exists() or manifest.exists():
        raise RuntimeError(f"refusing orphaned BOMB-credit6 training artifacts: {arm}/{replica}")
    parent = (ROOT / protocol["source_parent"]["path"]).resolve()
    spec = protocol["training"]
    seeds = spec["seeds_by_replica"][replica]
    command = [
        sys.executable, "main.py", "play", "--agents", "model_a_v4_bomb_credit6", *spec["opponents"],
    ...
    overrides = {
        ...
        "MODEL_A_BOMB_CREDIT6_CHECKPOINT_PATH": str(output),
        "MODEL_A_BOMB_CREDIT6_PARENT_PATH": str(parent),
        "MODEL_A_BOMB_CREDIT6_ARM": arm,
        "MODEL_A_BOMB_CREDIT6_REPLICA": replica,
        "MODEL_A_BOMB_CREDIT6_SEED": str(seeds["agent_seed"]),
    ...
    record = {
        ...
        "target_contract": {"bomb_maturation_steps": 6, "bomb_return_steps": 1 if arm == "control" else 6,
                            "non_bomb_return_steps": 1, "allowed_outcome_lags": [4, 5],
                            "terminal_event_delivery": "posthumous owned-bomb kills attach to the last terminal transition"},
    ...
    try:
        if completed.returncode != 0 or not stats_path.is_file():
            raise RuntimeError("BOMB-credit6 training process or stats failed")
    ...
    if record["status"] != "completed":
        raise RuntimeError(f"BOMB-credit6 training failed: {arm}/{replica}: {record.get('error')}")
    return record
...
def checkpoint_environment(protocol_path: Path, protocol: dict, label: str, case: dict) -> tuple[str, Path, dict[str, str]]:
    ...
    return "model_a_v4_bomb_credit6", checkpoint, {
        "MODEL_A_BOMB_CREDIT6_PROTOCOL_PATH": str(protocol_path),
        "MODEL_A_BOMB_CREDIT6_CHECKPOINT_PATH": str(checkpoint),
        "MODEL_A_BOMB_CREDIT6_ARM": arm, "MODEL_A_BOMB_CREDIT6_REPLICA": replica,
        "MODEL_A_BOMB_CREDIT6_SEED": str(case["agent_seed"]),
    }
...
def evaluate_one(protocol_path: Path, protocol: dict, protocol_hash: str, stratum: str, label: str, case: dict) -> dict:
    ...
    if existing is not None:
        if existing.get("protocol_sha256") != protocol_hash or existing.get("evaluation") != expected:
            raise RuntimeError(f"completed BOMB-credit6 evaluation drift: {output}")
        if existing.get("checkpoint", {}).get("sha256") != checkpoint_hash:
            raise RuntimeError(f"completed BOMB-credit6 evaluation checkpoint drift: {output}")
        return existing
    stats_path = output.with_suffix(".stats.json")
    if output.exists() or stats_path.exists():
        raise RuntimeError(f"refusing orphaned BOMB-credit6 evaluation: {output}")
    command = [
    ...
    atomic_json(output, record)
    if record["status"] != "completed":
        raise RuntimeError(f"BOMB-credit6 evaluation failed: {stratum}/{label}")
    return record
    ...
    for arm in ARMS:
        for replica in REPLICAS:
            ...
            target_checks[f"{arm}-{replica}"] = (
                ...
                and (arm == "control" or diagnostics["observed_kill_events"] >= rule["minimum_observed_kill_events_per_credit_replica"])
                and (
...
def write_report(protocol_path: Path, protocol: dict, protocol_hash: str, training: dict, rows: dict, manifests: dict, result: dict) -> tuple[dict, str]:
    ...
    if existing is not None:
        if existing.get("protocol_sha256") != protocol_hash:
            raise RuntimeError("completed BOMB-credit6 report protocol drift")
    ...
    record = {
        ...
        "default_checkpoint_modified": False, "failed_s130000_reused": False,
    }
...
def dry_run(protocol: dict) -> dict:
    ...
    evaluation_rounds = identities * sum(len(spec["cases"]) * int(spec["rounds_per_case"])
                                         for spec in protocol["evaluation"]["strata"].values())
    return {
        ...
        "control_bomb_return_steps": 1, "credit_bomb_return_steps": 6,
        "bomb_maturation_steps_both_arms": 6, "allowed_outcome_lags": [4, 5],
        "terminal_deferred_event_handling": True,
        "non_bomb_return_steps": 1, "fresh_s131000_seeds": True, "failed_s130000_reused": False,
...
def main(argv: list[str] | None = None) -> int:
    ...
    training = {arm: {replica: train_one(protocol_path.resolve(), protocol, protocol_hash, arm, replica)
                      for replica in REPLICAS} for arm in ARMS}
    if sha256_file(source_path) != source_before:
        raise RuntimeError("source-r2 changed during BOMB-credit6 training")
