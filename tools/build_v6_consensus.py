"""Build a validated v6s4a action-consensus bundle."""
...
from agent_code.model_a_v6_consensus.config import architecture_name, load_config, sha256_file
...
def main() -> int:
    ...
    if output.exists():
        raise SystemExit(f"refusing to overwrite consensus bundle: {output}")
    output.parent.mkdir(parents=True, exist_ok=True)
    torch.save(build(args.config.resolve()), output)
