"""Resume the frozen s162 control checkpoint for the s165 resource A/B."""
...
KIND = "model-a-cnn-n8-resource-competition-ab"
...
def setup(self) -> None:
    if torch is None:
        raise RuntimeError("resource A/B requires PyTorch")
    torch.set_num_threads(1)
    protocol_path = Path(_required("CNN_RESOURCE_PROTOCOL_PATH")).resolve()
    ...
    if self.cnn_protocol.get("kind") != KIND:
        raise RuntimeError("invalid resource A/B protocol")
    self.cnn_mode = os.environ.get("CNN_RESOURCE_MODE", "evaluate")
    self.cnn_stage = "task4"
    self.cnn_arm = _required("CNN_RESOURCE_ARM")
    self.cnn_replica = _required("CNN_RESOURCE_REPLICA")
    if self.cnn_arm not in self.cnn_protocol["arms"]:
        raise RuntimeError("unknown resource A/B arm")
    if self.cnn_replica not in self.cnn_protocol["replicas"]:
        raise RuntimeError("unknown resource A/B replica")
    self.agent_seed = int(_required("CNN_RESOURCE_AGENT_SEED"))
    if self.agent_seed != int(self.cnn_protocol["replicas"][self.cnn_replica]["agent_seed"]):
        raise RuntimeError("resource A/B learner seed mismatch")
    ...
    if self.train and self.cnn_mode == "train":
        ...
        if device_name == "mps" and not torch.backends.mps.is_available():
            raise RuntimeError("formal resource A/B requires MPS")
        self.device = torch.device(device_name)
    else:
        self.device = torch.device("cpu")
    checkpoint_path = Path(_required("CNN_RESOURCE_CHECKPOINT_PATH")).resolve()
    ...
    if self.cnn_mode == "train":
        ...
        if checkpoint_path != expected or _sha256(checkpoint_path) != parent["sha256"]:
            raise RuntimeError("resource A/B parent checkpoint drift")
    checkpoint = _load(checkpoint_path)
    if checkpoint.get("architecture") != ARCHITECTURE or checkpoint.get("stage") != "task4":
        raise RuntimeError("resource A/B checkpoint identity mismatch")
    if self.cnn_mode == "train":
        expected_identity = {
        ...
        if any(checkpoint.get(key) != value for key, value in expected_identity.items()):
            raise RuntimeError("resource A/B parent lineage mismatch")
