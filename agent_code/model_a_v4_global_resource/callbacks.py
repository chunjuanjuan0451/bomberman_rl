"""Callbacks for the stage-1 frozen-v4 global-resource experiment."""
...
from .config import ARMS, architecture_name, load_protocol
...
def setup(self):
    ...
    if self.global_run_mode == "train" and (
        self.global_arm not in ARMS or self.global_replica not in self.global_protocol["training"]["replicas"]
    ):
    ...
    if self.global_run_mode == "train":
        if os.environ.get("MODEL_A_GLOBAL_RESUME", "") == "1":
            raise RuntimeError("stage-1 global-resource training forbids resume")
        return
    ...
    if any(payload.get(key) != value for key, value in expected.items()):
        raise RuntimeError("global-resource checkpoint metadata mismatch")
