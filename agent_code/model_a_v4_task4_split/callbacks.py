"""Exact-v4 policy callbacks for Task4A open-rule training."""
...
def _torch_load(path: Path) -> dict:
    ...
    if not isinstance(payload, dict) or "online_net" not in payload:
        raise RuntimeError(f"invalid Task4A checkpoint: {path}")
    return payload
...
def _required(name: str) -> str:
    ...
    if not value:
        raise RuntimeError(f"{name} is required for model_a_v4_task4_split")
    return value
...
def setup(self):
    if torch is None:
        raise RuntimeError("model_a_v4_task4_split requires PyTorch")
    torch.set_num_threads(1)
    protocol_path = Path(_required("MODEL_A_V4T4S_PROTOCOL_PATH"))
    self.protocol, self.protocol_sha256 = load_protocol(protocol_path)
    self.replica = _required("MODEL_A_V4T4S_REPLICA")
    if self.replica not in REPLICAS:
        raise RuntimeError("invalid Task4A replica")
    supplied_seed = int(_required("MODEL_A_V4T4S_SEED"))
    expected_seed = int(self.protocol["training"]["seeds"][self.replica]["agent_seed"])
    if self.train and supplied_seed != expected_seed:
        raise RuntimeError("Task4A agent seed mismatch")
    ...
    supplied_checkpoint = Path(_required("MODEL_A_V4T4S_CHECKPOINT_PATH")).resolve()
    ...
    if self.train:
        ...
        if supplied_checkpoint != expected_output or supplied_checkpoint.exists():
            raise RuntimeError("Task4A output path mismatch or overwrite attempt")
        parent = Path(_required("MODEL_A_V4T4S_PARENT_PATH")).resolve()
        ...
        if parent != expected_parent or sha256_file(parent) != self.protocol["source_parent"]["sha256"]:
            raise RuntimeError("Task4A source-r2 parent mismatch")
        ...
        for key, value in expected_lineage.items():
            if payload.get(key) != value:
                raise RuntimeError(f"Task4A source parent {key} mismatch")
        ...
        if not supplied_checkpoint.is_file():
            raise FileNotFoundError(f"Task4A evaluation checkpoint missing: {supplied_checkpoint}")
        ...
        allowed = {
            ...
            *{snapshot_path(self.protocol, self.replica, round_number) for round_number in SNAPSHOT_ROUNDS},
        }
        if supplied_checkpoint not in allowed:
            raise RuntimeError("Task4A evaluation checkpoint is outside the immutable artifact set")
        self.online_net.load_state_dict(payload["online_net"], strict=True)
        self.completed_rounds = int(payload.get("completed_rounds", 0))
