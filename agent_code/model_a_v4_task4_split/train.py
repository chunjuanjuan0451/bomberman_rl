"""Unchanged exact-v4 learner with fine-grained Task4A snapshots."""
...
def end_of_round(self, last_game_state, last_action, events):
    ...
    if stage_round in self.protocol["snapshot_rounds"]:
        ...
        if snapshot.exists():
            raise RuntimeError(f"refusing to overwrite Task4A snapshot: {snapshot}")
        _atomic_save(_payload(self), snapshot)
        _atomic_copy(snapshot, self.checkpoint_path)
        self.logger.info("Saved split Task4A %s at round %d.", self.replica, stage_round)
