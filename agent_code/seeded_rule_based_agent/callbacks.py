"""Supplied rule-based policy with independent, reproducible RNG streams."""
...
def setup(self):
    seed_text = os.environ.get("TASK4_RULE_SEED")
    if seed_text is None:
        raise ValueError("seeded_rule_based_agent requires TASK4_RULE_SEED")
    callback_name = getattr(self, "name", self.logger.name)
    state = np.random.SeedSequence(
        [int(seed_text), _agent_index(callback_name), 0xB04B]
    ).generate_state(2)
    self.rng = random.Random((int(state[0]) << 32) | int(state[1]))
    reset_self(self)
...
def act(self, game_state):
    ...
    bomb_xys = [position for position, _ in bombs]
    ...
    columns = range(1, arena.shape[0] - 1)
    rows = range(1, arena.shape[0] - 1)
    dead_ends = [
        (column, row) for column in columns for row in rows
        if arena[column, row] == 0
        and [arena[column + 1, row], arena[column - 1, row],
             arena[column, row + 1], arena[column, row - 1]].count(0) == 1
    ]
    crates = [
        (column, row) for column in columns for row in rows if arena[column, row] == 1
    ]
    targets = coins + dead_ends + crates
    if self.ignore_others_timer <= 0 or not crates + coins:
    ...
    if others and min(abs(other[0] - x) + abs(other[1] - y) for other in others) <= 1:
        action_ideas.append("BOMB")
