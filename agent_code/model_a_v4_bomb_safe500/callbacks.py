"""Exact-v4 callbacks for the safe-kill-only BOMB reward-500 gate."""
...
from .config import ARMS, REPLICAS, ROOT, SNAPSHOT_ROUNDS, final_checkpoint_path, load_protocol, sha256_file, snapshot_path


def _required(name: str) -> str:
    value = os.environ.get(name)
    if not value:
        raise RuntimeError(f"{name} is required for model_a_v4_bomb_safe500")
    return value
...
def _torch_load(path: Path) -> dict:
    ...
    if not isinstance(payload, dict) or "online_net" not in payload:
        raise RuntimeError(f"invalid BOMB-safe500 checkpoint: {path}")
    return payload
...
def _validate_candidate(self, payload: dict, path: Path) -> None:
    expected = {
        ...
        "safe_kill_reward": 12.0 if self.arm == "safe12" else 500.0,
        "trade_kill_reward": 0.0, "bomb_return_steps": 6,
        "non_bomb_return_steps": 1, "bomb_maturation_steps": 6,
    }
...
def setup(self):
    if torch is None:
        raise RuntimeError("model_a_v4_bomb_safe500 requires PyTorch")
    torch.set_num_threads(1)
    protocol_path = Path(_required("MODEL_A_BOMB_SAFE500_PROTOCOL_PATH"))
    self.protocol, self.protocol_sha256 = load_protocol(protocol_path)
    self.arm = _required("MODEL_A_BOMB_SAFE500_ARM")
    self.replica = _required("MODEL_A_BOMB_SAFE500_REPLICA")
    if self.arm not in ARMS or self.replica not in REPLICAS:
        raise RuntimeError("invalid BOMB-safe500 arm or replica")
    supplied_seed = int(_required("MODEL_A_BOMB_SAFE500_SEED"))
    ...
    if self.train and supplied_seed != expected_seed:
        raise RuntimeError("BOMB-safe500 agent seed mismatch")
    ...
    supplied_checkpoint = Path(_required("MODEL_A_BOMB_SAFE500_CHECKPOINT_PATH")).resolve()
    if self.train:
        ...
        if supplied_checkpoint != expected_output or supplied_checkpoint.exists():
            raise RuntimeError("BOMB-safe500 output path mismatch or overwrite attempt")
        parent = Path(_required("MODEL_A_BOMB_SAFE500_PARENT_PATH")).resolve()
        ...
        if parent != expected_parent or sha256_file(parent) != self.protocol["source_parent"]["sha256"]:
            raise RuntimeError("BOMB-safe500 source-r2 parent mismatch")
        payload = _torch_load(parent)
        lineage = self.protocol["source_parent"]["lineage"]
        for key, value in {"architecture": MODEL_ARCHITECTURE, "protocol_sha256": lineage["protocol_sha256"],
                           "arm": "curriculum", "replica": "r2", "stage_id": "task2"}.items():
            if payload.get(key) != value:
                raise RuntimeError(f"BOMB-safe500 source parent {key} mismatch")
        ...
        if not supplied_checkpoint.is_file():
            raise FileNotFoundError(f"BOMB-safe500 evaluation checkpoint missing: {supplied_checkpoint}")
        payload = _torch_load(supplied_checkpoint)
        _validate_candidate(self, payload, supplied_checkpoint)
        allowed = {final_checkpoint_path(self.protocol, self.arm, self.replica), *{
            snapshot_path(self.protocol, self.arm, self.replica, value) for value in SNAPSHOT_ROUNDS
        }}
        if supplied_checkpoint not in allowed:
            raise RuntimeError("BOMB-safe500 checkpoint is outside the immutable artifact set")
        self.online_net.load_state_dict(payload["online_net"], strict=True)
        self.completed_rounds = int(payload.get("completed_rounds", 0))
