"""Unchanged exact-v4 learner with immutable Task4C snapshots."""
...
def end_of_round(self, last_game_state, last_action, events):
    ...
    if stage_round in self.protocol["snapshot_rounds"]:
        ...
        if snapshot.exists():
            raise RuntimeError(f"refusing to overwrite Task4C snapshot: {snapshot}")
        _atomic_save(_payload(self), snapshot)
        _atomic_copy(snapshot, self.checkpoint_path)
        self.logger.info("Saved direct Task4C %s at round %d.", self.replica, stage_round)
