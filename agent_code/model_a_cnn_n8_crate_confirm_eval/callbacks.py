"""Evaluate one fixed checkpoint in the s167 selection-free confirmation."""
...
def setup(self) -> None:
    protocol = json.loads(Path(os.environ["CNN_CRATE_CONFIRM_PROTOCOL_PATH"]).read_text(encoding="utf-8"))
    if protocol.get("kind") != KIND:
        raise RuntimeError("invalid low-crate confirmation protocol")
    path = Path(os.environ["CNN_CRATE_CONFIRM_CHECKPOINT"]).resolve()
    expected = os.environ["CNN_CRATE_CONFIRM_CHECKPOINT_SHA256"]
    if hashlib.sha256(path.read_bytes()).hexdigest() != expected:
        raise RuntimeError("low-crate confirmation checkpoint hash mismatch")
    payload = _load(path)
    if payload.get("architecture") != ARCHITECTURE or payload.get("stage") != "task4":
        raise RuntimeError("low-crate confirmation checkpoint identity mismatch")
    ...
    self.rng = np.random.default_rng(int(os.environ["CNN_CRATE_CONFIRM_AGENT_SEED"]))
