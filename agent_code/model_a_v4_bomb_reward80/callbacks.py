"""Exact-v4 callbacks for the six-step BOMB reward-scale signal gate."""
...
def _required(name: str) -> str:
    ...
    if not value:
        raise RuntimeError(f"{name} is required for model_a_v4_bomb_reward80")
    return value
...
def _torch_load(path: Path) -> dict:
    ...
    if not isinstance(payload, dict) or "online_net" not in payload:
        raise RuntimeError(f"invalid BOMB-reward80 checkpoint: {path}")
    return payload
...
def _validate_candidate(self, payload: dict, path: Path) -> None:
    expected = {
        ...
        "kill_reward": 12.0 if self.arm == "reward12" else 80.0,
        "bomb_return_steps": 6,
...
def setup(self):
    if torch is None:
        raise RuntimeError("model_a_v4_bomb_reward80 requires PyTorch")
    torch.set_num_threads(1)
    protocol_path = Path(_required("MODEL_A_BOMB_REWARD80_PROTOCOL_PATH"))
    self.protocol, self.protocol_sha256 = load_protocol(protocol_path)
    self.arm = _required("MODEL_A_BOMB_REWARD80_ARM")
    self.replica = _required("MODEL_A_BOMB_REWARD80_REPLICA")
    if self.arm not in ARMS or self.replica not in REPLICAS:
        raise RuntimeError("invalid BOMB-reward80 arm or replica")
    supplied_seed = int(_required("MODEL_A_BOMB_REWARD80_SEED"))
    ...
    if self.train and supplied_seed != expected_seed:
        raise RuntimeError("BOMB-reward80 agent seed mismatch")
    ...
    supplied_checkpoint = Path(_required("MODEL_A_BOMB_REWARD80_CHECKPOINT_PATH")).resolve()
    if self.train:
        ...
        if supplied_checkpoint != expected_output or supplied_checkpoint.exists():
            raise RuntimeError("BOMB-reward80 output path mismatch or overwrite attempt")
        parent = Path(_required("MODEL_A_BOMB_REWARD80_PARENT_PATH")).resolve()
        ...
        if parent != expected_parent or sha256_file(parent) != self.protocol["source_parent"]["sha256"]:
            raise RuntimeError("BOMB-reward80 source-r2 parent mismatch")
        ...
        for key, value in {
            ...
            if payload.get(key) != value:
                raise RuntimeError(f"BOMB-reward80 source parent {key} mismatch")
        ...
        if not supplied_checkpoint.is_file():
            raise FileNotFoundError(f"BOMB-reward80 evaluation checkpoint missing: {supplied_checkpoint}")
        ...
        if supplied_checkpoint not in allowed:
            raise RuntimeError("BOMB-reward80 checkpoint is outside the immutable artifact set")
        self.online_net.load_state_dict(payload["online_net"], strict=True)
        self.completed_rounds = int(payload.get("completed_rounds", 0))
