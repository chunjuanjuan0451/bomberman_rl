"""Frozen protocol helpers for the six-step BOMB reward-scale signal gate."""
...
SOURCE_PATHS = {
    ...
    "agent_code/model_a_v4_bomb_reward80/config.py",
    "agent_code/model_a_v4_bomb_reward80/callbacks.py",
    "agent_code/model_a_v4_bomb_reward80/train.py",
    "agent_code/seeded_rule_based_agent/callbacks.py",
    "tools/v4_task4_duel_training.py",
    "tools/v4_task4_bomb_reward80.py",
}
...
def load_protocol(path: Path) -> tuple[dict, str]:
    ...
    if protocol.get("schema_version") != 1 or protocol.get("kind") != "model-a-v4-task4-bomb-reward80":
        raise ValueError("wrong BOMB-reward80 protocol")
    if protocol.get("active_subcourse") != "task4c_three_rule":
        raise ValueError("BOMB-reward80 gate may run Task4C only")
    if tuple(protocol.get("arms", ())) != ARMS or tuple(protocol.get("replicas", ())) != REPLICAS:
        raise ValueError("BOMB-reward80 arms or replicas changed")
    if tuple(protocol.get("snapshot_rounds", ())) != SNAPSHOT_ROUNDS:
        raise ValueError("BOMB-reward80 snapshots changed")
    if protocol.get("single_training_variable") != "killed_opponent_reward_12_vs_80":
        raise ValueError("BOMB-reward80 experiment must change one variable")
    expected_learning = {
        ...
        "reward_difference": "only KILLED_OPPONENT is 12 versus 80",
        "all_other_reward_and_shaping": "unchanged exact-v4 values and events",
    ...
    if protocol.get("learning_contract") != expected_learning:
        raise ValueError("BOMB-reward80 learning contract changed")
    expected_target = {
        "gamma": 0.99,
        "bomb_maturation_steps_both_arms": 6,
        "bomb_return_steps_both_arms": 6,
        "non_bomb_return_steps_both_arms": 1,
    ...
    if protocol.get("target_contract") != expected_target:
        raise ValueError("BOMB-reward80 target contract changed")
    if protocol.get("reward_profiles") != {
        "reward12": {"KILLED_OPPONENT": 12.0},
        "reward80": {"KILLED_OPPONENT": 80.0},
    }:
        raise ValueError("BOMB-reward80 reward profiles changed")
    ...
    if (
        ...
        or int(training.get("rounds_per_arm", 0)) != 25
        or tuple(training.get("seeds_by_replica", {})) != REPLICAS
    ):
        raise ValueError("BOMB-reward80 training distribution or budget changed")
    ...
    if tuple(evaluation.get("strata", {})) != EVALUATION_STRATA:
        raise ValueError("BOMB-reward80 evaluation strata changed")
    for stratum, spec in evaluation["strata"].items():
        ...
        if spec.get("scenario") != scenario or spec.get("opponents") != opponents:
            raise ValueError(f"BOMB-reward80 evaluation distribution changed: {stratum}")
        if int(spec.get("rounds_per_case", 0)) != 25 or len(spec.get("cases", ())) != expected_cases:
            raise ValueError(f"BOMB-reward80 evaluation budget changed: {stratum}")
        for index, case in enumerate(spec["cases"]):
            _register(seen, case, f"evaluation/{stratum}/{index}")
    if len(seen) != 21:
        raise ValueError("BOMB-reward80 protocol must register exactly 21 unique seed values")

    expected_decision = {
        "minimum_supportive_pairs": 2,
        "minimum_observed_kill_events_per_replica": 1,
        "pair_minimum_task4c_kills_gain": 0.000000001,
        "pair_minimum_task4c_score_delta": -0.25,
        "pair_maximum_task4c_suicide_delta": 0.10,
        "pair_minimum_last_interval_bomb_fraction_of_control": 0.80,
        "candidate_minimum_last_interval_bomb_fraction_of_first": 0.80,
        ...
        "pass_decision": "reward80_signal_supported_stop_before_expansion",
        "fail_decision": "reward80_signal_not_supported_stop",
    }
    if evaluation.get("decision_rule") != expected_decision:
        raise ValueError("BOMB-reward80 decision rule changed")
    if protocol.get("automatic_followup") is not False:
        raise ValueError("BOMB-reward80 gate must always stop")

    if set(protocol.get("source_bindings", {})) != SOURCE_PATHS:
        raise ValueError("BOMB-reward80 source binding set mismatch")
    for relative_path, expected_hash in protocol["source_bindings"].items():
        source = ROOT / relative_path
        if not source.is_file() or sha256_file(source) != expected_hash:
            raise ValueError(f"BOMB-reward80 source binding mismatch: {relative_path}")
    for field in ("source_parent", "frozen_v4_baseline", "prior_six_step_report"):
        ...
        if not artifact.is_file() or sha256_file(artifact) != item.get("sha256"):
            raise ValueError(f"BOMB-reward80 bound artifact mismatch: {field}")
    prior = json.loads((ROOT / protocol["prior_six_step_report"]["path"]).read_text(encoding="utf-8"))
    if prior.get("result", {}).get("decision") != "bomb_six_step_credit_not_supported_stop":
        raise ValueError("prior six-step evidence changed")
    ...
    if not lineage_protocol.is_file() or sha256_file(lineage_protocol) != lineage.get("protocol_sha256"):
        raise ValueError("BOMB-reward80 source lineage changed")
    return protocol, sha256_file(path)


def checkpoint_root(protocol: dict, arm: str, replica: str) -> Path:
    if arm not in ARMS or replica not in REPLICAS:
        raise ValueError("invalid BOMB-reward80 checkpoint identity")
...
def snapshot_path(protocol: dict, arm: str, replica: str, stage_round: int) -> Path:
    if stage_round not in SNAPSHOT_ROUNDS:
        raise ValueError("invalid BOMB-reward80 snapshot round")
