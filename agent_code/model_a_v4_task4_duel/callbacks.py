"""Exact-v4 policy callbacks for isolated Task4 duel training."""
...
def _torch_load(path: Path) -> dict:
    ...
    if not isinstance(payload, dict) or "online_net" not in payload:
        raise RuntimeError(f"invalid Task4 duel checkpoint: {path}")
    return payload
...
def _required(name: str) -> str:
    ...
    if not value:
        raise RuntimeError(f"{name} is required for model_a_v4_task4_duel")
    return value
...
def setup(self):
    if torch is None:
        raise RuntimeError("model_a_v4_task4_duel requires PyTorch")
    torch.set_num_threads(1)
    protocol_path = Path(_required("MODEL_A_V4T4D_PROTOCOL_PATH"))
    self.protocol, self.protocol_sha256 = load_protocol(protocol_path)
    self.replica = _required("MODEL_A_V4T4D_REPLICA")
    if self.replica not in REPLICAS:
        raise RuntimeError("invalid Task4 replica")
    supplied_seed = int(_required("MODEL_A_V4T4D_SEED"))
    expected_seed = int(self.protocol["training"]["seeds"][self.replica]["agent_seed"])
    if self.train and supplied_seed != expected_seed:
        raise RuntimeError("Task4 agent seed mismatch")
    ...
    supplied_checkpoint = Path(_required("MODEL_A_V4T4D_CHECKPOINT_PATH")).resolve()
    ...
    if self.train:
        ...
        if supplied_checkpoint != expected_output or supplied_checkpoint.exists():
            raise RuntimeError("Task4 output path mismatch or overwrite attempt")
        parent = Path(_required("MODEL_A_V4T4D_PARENT_PATH")).resolve()
        ...
        if parent != expected_parent or sha256_file(parent) != self.protocol["source_parent"]["sha256"]:
            raise RuntimeError("Task4 source-r2 parent mismatch")
        ...
        for key, value in expected_lineage.items():
            if payload.get(key) != value:
                raise RuntimeError(f"Task4 source parent {key} mismatch")
        ...
        if not supplied_checkpoint.is_file():
            raise FileNotFoundError(f"Task4 evaluation checkpoint missing: {supplied_checkpoint}")
        ...
        if supplied_checkpoint not in allowed:
            raise RuntimeError("Task4 evaluation checkpoint is outside the immutable artifact set")
        self.online_net.load_state_dict(payload["online_net"], strict=True)
        self.completed_rounds = int(payload.get("completed_rounds", 0))
