"""Official callbacks for the Task-3 four-arm residual agent."""
...
CHECKPOINT_PATH = Path(os.environ.get("MODEL_A_TASK3_CHECKPOINT_PATH", "model_a_task3.pt"))
...
def setup(self):
    ...
    seed_text = os.environ.get("MODEL_A_TASK3_SEED")
    if seed_text is None:
        raise RuntimeError("MODEL_A_TASK3_SEED is required")
    self.agent_seed = int(seed_text)
    torch.manual_seed(self.agent_seed)
    self.rng = np.random.default_rng(self.agent_seed)
    self.device = torch.device("cpu")
    self.online_net = Task3DQN(
        hyper["delta_cap"], self.task3_config["action_features"],
    ).to(self.device)
    self.parent_checkpoint_path = _load_parent(
        self.online_net, self.task3_config, self.task3_config_path,
    )
    if os.environ.get("MODEL_A_TASK3_RESUME", "") == "1":
        raise RuntimeError("Task-3 four-arm runs forbid resume")
    if self.train:
    ...
    if checkpoint.get("arm") != self.task3_config["arm"]:
        raise RuntimeError("Task-3 checkpoint arm mismatch")
...
def act(self, game_state: dict) -> str:
    ...
    action_features = task3_action_features(game_state) if self.task3_config["action_features"] else None
    with torch.no_grad():
        q_values = self.online_net(
            ...
            None if action_features is None else torch.as_tensor(
                action_features[None], dtype=torch.float32, device=self.device,
            ),
        )[0].cpu().numpy()
