...
def _path_distances(
    ...
    """Shortest path distances over currently free tiles."""
    from collections import deque
    ...
    while queue:
        ...
        for dx, dy in DELTAS.values():
            candidate = position[0] + dx, position[1] + dy
            if candidate in distances or candidate in blocked:
                continue
            if not _in_bounds(field, candidate) or field[candidate] != 0:
                continue
            distances[candidate] = distances[position] + 1
            queue.append(candidate)
    return distances


def _opponent_threat_from_bomb(game_state: dict) -> float:
    """Estimate direct threat using the opponent's remaining escape space."""
    field = game_state["field"]
    start = _position(game_state)
    own_blast = set(blast_positions(field, start))
    opponents = [tuple(other[3]) for other in game_state["others"]]
    exposed = [opponent for opponent in opponents if opponent in own_blast]
    if not exposed:
        return 0.0
    static_blocked = {tuple(position) for position, _ in game_state["bombs"]}
    static_blocked |= set(opponents) | {start}
    scores = []
            ...
            for position in positions:
                for dx, dy in DELTAS.values():
                    ...
                    if _in_bounds(field, candidate) and field[candidate] == 0:
                        next_positions.add(candidate)
            positions = next_positions
        safe_positions = positions - own_blast
        scores.append(1.0 if not safe_positions else 1.0 / (1.0 + len(safe_positions)))
...
def _survival_space(
    ...
    """Return bounded reachable safe space after a short time-space rollout."""
    if start_time in hazards.get(start, set()):
        return 0.0
    field = game_state["field"]
    blocked = _blocked_positions(game_state) | (extra_blocked or set())
    positions = {start}
    for time in range(start_time + 1, max(horizon, start_time + 3) + 1):
        next_positions = set()
        for position in positions:
            candidates = [position]
            candidates.extend((position[0] + dx, position[1] + dy) for dx, dy in DELTAS.values())
        ...
        if not positions:
            return 0.0
    return min(1.0, len(positions) / 8.0)


def action_tactical_features(game_state: dict | None) -> np.ndarray:
    """Return bounded [action, coin, crate, escape, threat] features.

    Coin progress uses traversable shortest paths. Crate value represents a
    counterfactual bomb at the action destination. BOMB escape includes the
    newly placed bomb, and threat measures the exposed opponent's remaining
    escape space rather than duplicating line-of-sight alone.
    """
    ...
    start_distances = _path_distances(field, start, blocked - {start})
    start_coin_distance = min((start_distances.get(coin, INF_TIME) for coin in coins), default=INF_TIME)
    ...
    for index, action in enumerate(ACTIONS):
        ...
        destination_distances = _path_distances(field, destination, blocked - {destination})
        destination_coin_distance = min(
            (destination_distances.get(coin, INF_TIME) for coin in coins), default=INF_TIME,
        )
        if start_coin_distance < INF_TIME and destination_coin_distance < INF_TIME:
            values[index, 0] = float(np.clip(start_coin_distance - destination_coin_distance, -1, 1))

        if game_state["self"][2]:
            hypothetical_blast = blast_positions(field, destination)
            values[index, 1] = min(
                1.0, sum(field[position] == 1 for position in hypothetical_blast) / 4.0,
            )

        if action == "BOMB":
            bomb_hazards, bomb_horizon = _hazard_schedule(game_state, (start, BOMB_TIMER))
            survival_space = _survival_space(
                game_state, start, 1, bomb_hazards, bomb_horizon, {start},
            )
            values[index, 3] = _opponent_threat_from_bomb(game_state)
        else:
            survival_space = _survival_space(game_state, destination, 1, hazards, horizon)
        values[index, 2] = survival_space
    return values
