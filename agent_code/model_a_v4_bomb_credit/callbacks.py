"""Exact-v4 callbacks for the BOMB temporal-credit matched A/B."""
...
def _required(name: str) -> str:
    ...
    if not value:
        raise RuntimeError(f"{name} is required for model_a_v4_bomb_credit")
    return value
...
def _torch_load(path: Path) -> dict:
    ...
    if not isinstance(payload, dict) or "online_net" not in payload:
        raise RuntimeError(f"invalid BOMB-credit checkpoint: {path}")
    return payload
...
def _validate_candidate(self, payload: dict, path: Path) -> None:
    expected = {
        ...
        "bomb_return_steps": 1 if self.arm == "control" else 5,
...
def setup(self):
    if torch is None:
        raise RuntimeError("model_a_v4_bomb_credit requires PyTorch")
    torch.set_num_threads(1)
    protocol_path = Path(_required("MODEL_A_BOMB_CREDIT_PROTOCOL_PATH"))
    self.protocol, self.protocol_sha256 = load_protocol(protocol_path)
    self.arm = _required("MODEL_A_BOMB_CREDIT_ARM")
    self.replica = _required("MODEL_A_BOMB_CREDIT_REPLICA")
    if self.arm not in ARMS or self.replica not in REPLICAS:
        raise RuntimeError("invalid BOMB-credit arm or replica")
    supplied_seed = int(_required("MODEL_A_BOMB_CREDIT_SEED"))
    ...
    if self.train and supplied_seed != expected_seed:
        raise RuntimeError("BOMB-credit agent seed mismatch")
    ...
    supplied_checkpoint = Path(_required("MODEL_A_BOMB_CREDIT_CHECKPOINT_PATH")).resolve()
    if self.train:
        ...
        if supplied_checkpoint != expected_output or supplied_checkpoint.exists():
            raise RuntimeError("BOMB-credit output path mismatch or overwrite attempt")
        parent = Path(_required("MODEL_A_BOMB_CREDIT_PARENT_PATH")).resolve()
        ...
        if parent != expected_parent or sha256_file(parent) != self.protocol["source_parent"]["sha256"]:
            raise RuntimeError("BOMB-credit source-r2 parent mismatch")
        ...
        for key, value in {
            ...
            if payload.get(key) != value:
                raise RuntimeError(f"BOMB-credit source parent {key} mismatch")
        ...
        if not supplied_checkpoint.is_file():
            raise FileNotFoundError(f"BOMB-credit evaluation checkpoint missing: {supplied_checkpoint}")
        ...
        if supplied_checkpoint not in allowed:
            raise RuntimeError("BOMB-credit checkpoint is outside the immutable artifact set")
        self.online_net.load_state_dict(payload["online_net"], strict=True)
        self.completed_rounds = int(payload.get("completed_rounds", 0))
