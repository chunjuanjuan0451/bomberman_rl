"""Frozen protocol helpers for the BOMB temporal-credit matched A/B."""
...
SOURCE_PATHS = {
    ...
    "agent_code/model_a_v4_bomb_credit/config.py",
    "agent_code/model_a_v4_bomb_credit/callbacks.py",
    "agent_code/model_a_v4_bomb_credit/replay.py",
    "agent_code/model_a_v4_bomb_credit/train.py",
    ...
    "tools/v4_bomb_credit_attribution_audit.py",
    "tools/v4_task4_bomb_credit.py",
}
...
def load_protocol(path: Path) -> tuple[dict, str]:
    ...
    if protocol.get("schema_version") != 1 or protocol.get("kind") != "model-a-v4-task4-bomb-credit":
        raise ValueError("wrong BOMB-credit protocol")
    if protocol.get("active_subcourse") != "task4c_three_rule":
        raise ValueError("BOMB-credit pilot may run Task4C only")
    if tuple(protocol.get("arms", ())) != ARMS or tuple(protocol.get("replicas", ())) != REPLICAS:
        raise ValueError("BOMB-credit arms or replicas changed")
    if tuple(protocol.get("snapshot_rounds", ())) != SNAPSHOT_ROUNDS:
        raise ValueError("BOMB-credit snapshots changed")
    if protocol.get("single_training_variable") != "bomb_transition_return_horizon_1_vs_5":
        raise ValueError("BOMB-credit experiment must change one variable")
    expected_learning = {
    ...
    if protocol.get("learning_contract") != expected_learning:
        raise ValueError("BOMB-credit learning contract changed")
    expected_target = {
        "gamma": 0.99,
        "bomb_maturation_steps_both_arms": 5,
        "control_bomb_return_steps": 1,
        "credit_bomb_return_steps": 5,
        "event_lag_steps": 4,
        "terminal_truncation": "same-episode only",
    }
    if protocol.get("target_contract") != expected_target:
        raise ValueError("BOMB-credit target contract changed")
    ...
    if (
        ...
        raise ValueError("BOMB-credit training distribution or budget changed")
    ...
    if tuple(evaluation.get("strata", {})) != EVALUATION_STRATA:
        raise ValueError("BOMB-credit evaluation strata changed")
    for stratum, spec in evaluation["strata"].items():
        ...
        if spec.get("scenario") != scenario or spec.get("opponents") != opponents:
            raise ValueError(f"BOMB-credit evaluation distribution changed: {stratum}")
        if int(spec.get("rounds_per_case", 0)) != 25 or len(spec.get("cases", ())) != expected_cases:
            raise ValueError(f"BOMB-credit evaluation budget changed: {stratum}")
        for index, case in enumerate(spec["cases"]):
            _register(seen, case, f"evaluation/{stratum}/{index}")
    if len(seen) != 33:
        raise ValueError("BOMB-credit protocol must register exactly 33 unique seed values")
    ...
    expected_decision = {
        ...
        "pass_decision": "bomb_five_step_credit_supported_stop_before_task4b",
        "fail_decision": "bomb_five_step_credit_not_supported_stop",
    }
    if evaluation.get("decision_rule") != expected_decision:
        raise ValueError("BOMB-credit decision rule changed")
    if protocol.get("automatic_followup") is not False:
        raise ValueError("BOMB-credit experiment must always stop")

    if set(protocol.get("source_bindings", {})) != SOURCE_PATHS:
        raise ValueError("BOMB-credit source binding set mismatch")
    for relative_path, expected_hash in protocol["source_bindings"].items():
        source = ROOT / relative_path
        if not source.is_file() or sha256_file(source) != expected_hash:
            raise ValueError(f"BOMB-credit source binding mismatch: {relative_path}")
    for field in ("source_parent", "frozen_v4_baseline", "attribution_audit"):
        ...
        if not artifact.is_file() or sha256_file(artifact) != item.get("sha256"):
            raise ValueError(f"BOMB-credit bound artifact mismatch: {field}")
    audit = json.loads((ROOT / protocol["attribution_audit"]["path"]).read_text(encoding="utf-8"))
    if audit.get("decision") != "fixed_four_step_bomb_outcome_lag_confirmed":
        raise ValueError("BOMB-credit attribution audit did not pass")
...
def snapshot_path(protocol: dict, arm: str, replica: str, stage_round: int) -> Path:
    if stage_round not in SNAPSHOT_ROUNDS:
        raise ValueError("invalid BOMB-credit snapshot round")
