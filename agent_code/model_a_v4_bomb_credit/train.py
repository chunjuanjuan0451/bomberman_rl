"""Exact-v4 learner changing only the return horizon of BOMB transitions."""
...
def aggregate_return(window: list[RawTransition], return_steps: int, gamma: float) -> Transition:
    """Aggregate at most ``return_steps`` same-episode transitions."""
...
def setup_training(self):
    ...
    if checkpoint is None:
        raise RuntimeError("BOMB-credit experiment must resume immutable source-r2")
    ...
    if abs(self.epsilon - v4.EPSILON_FINAL) > 1e-12:
        raise RuntimeError("BOMB-credit parent must already be at the exact-v4 epsilon floor")
    self.bomb_return_steps = 1 if self.arm == "control" else 5
    ...
    self.logger.info(
        "BOMB-credit %s/%s: BOMB return=%d, non-BOMB return=1, maturation=5.",
...
def _assert_outcome_attribution(self, raw: RawTransition) -> None:
    for event, observed_key, attributed_key in (
        ...
        matches = [
            ...
            and window[0].start_step == raw.start_step - 4
        ]
        if len(matches) != 1:
            self.target_diagnostics["attribution_errors"] += count
            raise RuntimeError(
                f"official {event} at {raw.episode_id}/{raw.start_step} did not uniquely match step-4 BOMB"
            )
        self.target_diagnostics[attributed_key] += count


def _mature_bomb(self, window: list[RawTransition]) -> None:
    selected_steps = 1 if self.arm == "control" else min(5, len(window))
    ...
    diagnostics["bomb_targets_full_five_step_observed"] += int(len(window) >= 5)
    diagnostics["bomb_targets_terminal_truncated"] += int(len(window) < 5)
...
def end_of_round(self, last_game_state, last_action, events):
    ...
    if self._pending_bombs:
        raise RuntimeError("BOMB-credit pending queue crossed an episode boundary")
    ...
    if stage_round in self.protocol["snapshot_rounds"]:
        ...
        if snapshot.exists():
            raise RuntimeError(f"refusing to overwrite BOMB-credit snapshot: {snapshot}")
        _atomic_save(_payload(self), snapshot)
        _atomic_copy(snapshot, self.checkpoint_path)
        self.logger.info("Saved BOMB-credit %s/%s round %d.", self.arm, self.replica, stage_round)
