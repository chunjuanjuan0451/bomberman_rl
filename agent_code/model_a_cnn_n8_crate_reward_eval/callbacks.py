"""Evaluate one immutable s166 crate-reward A/B checkpoint."""
...
def setup(self) -> None:
    protocol = json.loads(Path(os.environ["CNN_CRATE_PROTOCOL_PATH"]).read_text(encoding="utf-8"))
    if protocol.get("kind") != "model-a-cnn-n8-crate-reward-ab":
        raise RuntimeError("invalid crate-reward A/B evaluation protocol")
    path = Path(os.environ["CNN_CRATE_EVAL_CHECKPOINT"]).resolve()
    expected = os.environ["CNN_CRATE_EVAL_CHECKPOINT_SHA256"]
    if hashlib.sha256(path.read_bytes()).hexdigest() != expected:
        raise RuntimeError("crate-reward A/B evaluation checkpoint hash mismatch")
    payload = _load(path)
    if payload.get("architecture") != ARCHITECTURE or payload.get("stage") != "task4":
        raise RuntimeError("crate-reward A/B evaluation checkpoint identity mismatch")
    ...
    self.rng = np.random.default_rng(int(os.environ["CNN_CRATE_EVAL_AGENT_SEED"]))
